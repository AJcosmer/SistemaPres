...
export function renderizarTablaEquipos(equipos, onEditar, onEliminar) {
  ...
  equipos.forEach(eq => {
    ...
    tr.innerHTML = `
      ...
      <td>${eq.TIPO}</td>
      <td>${eq.SERIAL || '-'}</td>
      <td><strong class="estado-${(eq.ESTADO || '').toLowerCase()}">${eq.ESTADO}</strong></td>
...
export function cargarSelectDisponibles(equipos) {
  ...
  disponibles.forEach(e => {
    ...
    opt.value = e.ID;
    opt.textContent = `${e.NOMBRE} (${e.SERIAL || 'Sin Serial'})`;
...
export function renderizarTablaPrestamos(prestamos, onDevolver) {
  ...
  prestamos.forEach(p => {
    const fechaP = p.FECHA_PRESTAMO ? p.FECHA_PRESTAMO.split('T')[0] : '-';
    const fechaD = p.FECHA_DEVOLUCION_PREVISTA ? p.FECHA_DEVOLUCION_PREVISTA.split('T')[0] : '-';
    const nombreEquipo = p.EQUIPO || p.NOMBRE_EQUIPO || `Equipo #${p.EQUIPO_ID}`;
    ...
    tr.innerHTML = `
      ...
      <td>${fechaP}</td>
      <td>${fechaD}</td>
      <td><strong class="estado-${(p.ESTADO || '').toLowerCase()}">${p.ESTADO}</strong></td>
      <td>
        ${p.ESTADO === 'ACTIVO' 
